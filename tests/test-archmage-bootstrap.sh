...
echo "    • In archmage-bootstrap.sh: comment out the 'Production' CONFIG"
echo "      block, uncomment the 'Test' block, and set LUKS_DEV to the loop"
echo "      device above (may not be /dev/loop0 if others are attached)."
echo "    • Also swap the PREFLIGHT command loop: comment out the production"
echo "      one (needs pacstrap/genfstab/arch-chroot) and uncomment the"
echo "      shorter testing-only one."
echo "    • Comment out the EFI mount and Steps 9–16 (marked in the script) —"
echo "      no fake EFI partition, package mirror, or chrootable system exists"
echo "      here. This harness only exercises Steps 1–8: LUKS, LVM, home-"
echo "      subvolume detection, and the format/mount logic."
